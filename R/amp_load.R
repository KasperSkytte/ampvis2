...
amp_load <- function(otutable,
  ...
  if (!is.null(metadata)) {
    ...
    metadata <- data.frame(
      "SampleID" = colnames(otutable[, 1:(ncol(otutable) - 7), drop = FALSE]), check.names = FALSE,
  ...
  if (!all(tax.names %in% expected.tax)) {
    stop(paste(
      "The last 7 columns in the OTU-table must be the taxonomy (Kingdom->Species) and named accordingly\nCurrent:",
  ...
  tax <- data.frame(
    otutable[, (ncol(otutable) - 6):ncol(otutable), drop = FALSE],
    OTU = rownames(otutable)
  ) # with added OTU column
