...
```{r, eval = FALSE}
myotutable <- read.delim("data/otutable.csv", 
                        #sep = "\t",
                        #dec = ".",
                        fill = TRUE,
                        header = TRUE,
                        ...
                        check.names = FALSE,
                        row.names = 1)
#note: If loading metadata from a csv file, DO NOT use row.names = 1. It's a must with otutables, however. 
mymetadata <- readxl::read_excel("data/metadata.xlsx",
                         col_names = TRUE)
```

Note that there are multiple types of CSV files. To change the separator to fx a comma you can adjust with the argument `sep = ","`. Refer to the documentation of `?read.table`.
...
# Other functions
There are numerous other functions to try out. Go explore them in the [Reference](../reference/index.html) tab.
