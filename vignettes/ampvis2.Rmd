...
## Loading data
ampvis2 imports data directly from any of the commonly used [QIIME](http://qiime.org/), [mothur](https://www.mothur.org/), and [UPARSE](http://www.drive5.com/uparse/) amplicon pipelines. Both QIIME and mothur (through the [make.biom](https://www.mothur.org/wiki/Make.biom) script) can export data in the [BIOM format](http://biom-format.org/) and can be loaded with the [amp_import_biom](https://madsalbertsen.github.io/ampvis2/reference/amp_import_biom.html) function. Data from the UPARSE pipeline can be imported by the [amp_import_uparse](https://madsalbertsen.github.io/ampvis2/reference/amp_import_usearch.html) function. The data is then loaded into a single `ampvis2` object by the `amp_load()` function, which results in a list of data frames, see the below illustration of the data structure and typical workflow of ampvis2:

![](workflow.png)

To demonstrate how to load data from raw CSV files download and unzip this [minimal example data](https://doi.org/10.6084/m9.figshare.6139352). The otutable.csv and metadata.csv files can then be imported into R by an appropriate read function:
...
Note that there are multiple types of CSV files. To change the separator to fx a comma you can adjust with the argument `sep = ","`. Refer to the documentation of [read.table](https://www.rdocumentation.org/packages/utils/versions/3.4.1/topics/read.table). To load excel files use the `readxl::read_excel` function instead. 
