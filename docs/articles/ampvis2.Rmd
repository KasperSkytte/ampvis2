...
```{r, warning = FALSE}
data("MiD")
MiDsubset <- amp_subset_samples(MiD, Plant %in% c("Aalborg West", "Aalborg East"))
```
...
```{r}
MiDsubset <- amp_subset_samples(MiD, Plant %in% c("Aalborg West", "Aalborg East") & !SeqID %in% c("16SAMP-749"), minreads = 10000)
```
...
```{r}
MiD_Chloroflexi_Actinobacteria <- amp_subset_taxa(MiD, tax_vector=c("p__Chloroflexi", "p__Actinobacteria"))
```
...
```{r}
amp_heatmap(MiDsubset, group = "Plant")
```
...
```{r, message=FALSE, warning=FALSE}
ordinationresult <- amp_ordinate(MiDsubset, 
             type = "CCA",
             constrain = "Period",
             transform = "Hellinger",
